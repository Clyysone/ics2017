...
void difftest_step(uint32_t eip) {
  ...
  // Set `diff` as `true` if they are not the same.
    ...
    {
      diff = true;
      printf("qemu eax:0x%08x,mine eax:0x%08x @eip:0x%08x\n",r.eax,mine.eax,mine.eip);
      //printf("qemu ebx:0x%08x,mine ebx:0x%08x @eip:0x%08x\n",r.ebx,mine.ebx,mine.eip);
    }  
    if(r.ecx != mine.ecx)
    {
      diff = true;
      printf("qemu ecx:0x%08x,mine ecx:0x%08x @eip:0x%08x\n",r.ecx,mine.ecx,mine.eip);
    }    
    if(r.edx != mine.edx)
    {
      diff = true;
      printf("qemu edx:0x%08x,mine edx:0x%08x @eip:0x%08x\n",r.edx,mine.edx,mine.eip);
    } 
    if(r.ebx != mine.ebx) 
    {
      diff = true;
    ...
    {
      diff = true;
      printf("qemu esp:0x%08x,mine esp:0x%08x @eip:0x%08x\n",r.esp,mine.esp,mine.eip);
    } 
    if(r.ebp != mine.ebp)
    {
      diff = true;
      printf("qemu ebp:0x%08x,mine ebp:0x%08x @eip:0x%08x\n",r.ebp,mine.ebp,mine.eip);
    } 
    if(r.esi != mine.esi)
    {
      diff = true;
      printf("qemu esi:0x%08x,mine esi:0x%08x @eip:0x%08x\n",r.esi,mine.esi,mine.eip);
    }  
    if(r.edi != mine.edi)
    {
      diff = true;
      printf("qemu edi:0x%08x,mine edi:0x%08x @eip:0x%08x\n",r.edi,mine.edi,mine.eip);
    } 
    if(r.eip != mine.eip)
    {
      diff = true;
      printf("qemu eip:0x%08x,mine eip:0x%08x @eip:0x%08x\n",r.eip,mine.eip,mine.eip);
    } 
    /*if((r.eflags&0x00000080) != (mine.eflags&0x00000080))
    {
      diff = true;
      printf("qemu eflags:0x%08x,mine eflags:0x%08x @eip:0x%08x\n",r.eflags,mine.eflags,mine.eip);
    } */
